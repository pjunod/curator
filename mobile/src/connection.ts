...
export function normalizeServerUrl(input: string): string {
  let raw = input.trim()
  if (!raw) throw new Error('Enter your Curator server address.')
  if (/^[a-z][a-z0-9+.-]*:\/\//i.test(raw) && !/^https?:\/\//i.test(raw)) {
    throw new Error('Curator server addresses must use http:// or https://.')
  }
  ...
  // parsing. A direct Curator host with no port should use Curator's own default
